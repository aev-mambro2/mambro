...
::Version: 2019-11-05T09:44:00EST
...
for /f "tokens=* USEBACKQ" %%A in (`dir /b /a-d "%~2\%~3"`) do (
  call :archive_file "%myScriptName%" "%~2" "%%A" "%pathTo7Zip%" "%zipNamePrefix%"
)
...
if %isDebugging% NEQ 0 (
  echo %date%T%time% Info from %~1: Examining file %~2\%~3 for archiving.
  echo.
)
SETLOCAL 
  set /A yearOfFile=0
