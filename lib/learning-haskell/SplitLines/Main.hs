...
-- ^ the pattern for a file path.

-- ^ the sequence number to put into the output file name.
...
-- ^ records, output file pattern. Records should be a list of items, each of which is a record that needs to get saved into its own file. Output file pattern: a pattern for naming a new output file. Will be suffixed with -#.txt in which # equals a sequence number.
